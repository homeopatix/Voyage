...
function GenerateOptionsWindow()
		...
		OptionsWindow.Message:SetText("Max " .. NBR_MaxLines); 
		...
		OptionsWindow.Message:SetTextAlignment(Turbine.UI.ContentAlignment.MiddleCenter); 
		OptionsWindow.Message:SetFont(Turbine.UI.Lotro.Font.Verdana12);
		OptionsWindow.Message:SetText("Min 3 - Max " .. NBR_MaxSlots); 
...
function ValidateChanges()
	buttonValider.MouseClick = function(sender, args)
			...
			if(tmpLine < 1 or tmpLine > NBR_MaxLines)then
				tmpLine = NBR_MaxLines;
			...
			if(tmpSlots > 20 or tmpSlots < 3 or (tmpSlots*settings["nbrLine"]["nbr"]) > NBR_Teleports)then
				tmpSlots = 10;
