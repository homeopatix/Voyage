...
function Debugage(value)
	...
	local mariner = 0;
	mariner = settings["marinerLoc"]["nbr"];
	local reput = 0;
	...
	Turbine.Shell.WriteLine("\n---- Debug part " .. tostring(value) .. " ----\n");
	Turbine.Shell.WriteLine("valHome : " .. travHome);
	...
	Turbine.Shell.WriteLine("mariner : " .. mariner);
	Turbine.Shell.WriteLine("reput : " .. reput);
	Turbine.Shell.WriteLine("race : " .. race);
	Turbine.Shell.WriteLine("class : " .. classP);
	Turbine.Shell.WriteLine("NbLines : " .. tostring(settings["nbrLine"]["nbr"]));
	Turbine.Shell.WriteLine("NbSlots : " .. tostring(settings["nbrSlots"]["nbr"]));
	local nbslots = tonumber(settings["nbrSlots"]["nbr"]);
	local nblines = tonumber(settings["nbrLine"]["nbr"])
	Turbine.Shell.WriteLine("TotalSlots : " .. tostring(nbslots * nblines));
end
------------------------------------------------------------------------------------------
...
function SetDatasFreePeople()
	...
	for i=(tele+valHome), (tele+valHome+racial) do
		if(settings["shortcuts"]["Data" .. i] == "") then
			...
			if(race == 125)then
				settings["shortcuts"]["Data" .. i] = racialLocations[8];
			end
		end
	end
	...
	if(classP == 216)then  -- mariner
		for i=(tele+valHome+racial), (tele+valHome+racial+mariner) do
			if(settings["shortcuts"]["Data" .. i] == "") then
				settings["shortcuts"]["Data" .. i] = marinerLocations[i-(tele+valHome+racial)];
			end
		end
		if(settings["displayReput"]["value"] == true)then
			for i=(tele+valHome+racial+mariner), (tele+valHome+racial+reput+mariner) do
				if(settings["shortcuts"]["Data" .. i] == "") then
					settings["shortcuts"]["Data" .. i] = reputLocations[i-((tele+valHome+racial+mariner))];
				end
			end
		end
	end
...
function DisplayInfosWindow(posx, posy)
	...
	ButtonInfos:SetParent( VoyageWindow );
