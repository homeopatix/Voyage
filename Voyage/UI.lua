...
if(settings["FirstInitialization"]["value"] == true)then
	...
	classP == 23 or
	classP == 172)then
		settings["nbrLine"]["nbr"] = NBR_MaxLines;
	end
	if(classP == 194)then
		settings["nbrLine"]["nbr"] = NBR_MaxLines;
	end
...
function GenerateWindow()
	...
	------------------------------------------------------------------------------------------
	DisplayInfosWindow(windowWidth/2 - 75, windowHeight - 20);
end
------------------------------------------------------------------------------------------
			...
			if (args.Button == Turbine.UI.MouseButton.Right) then
				if(settings["isMapWindowVisible"]["value"] == false)then
					if( CheckRacial(centerQS[i]:GetShortcut():GetData()))then
						--Turbine.Shell.WriteLine("parametre 1 : " .. settings["shortcuts"]["Data" .. i]);
						--Turbine.Shell.WriteLine("parametre 2 : " .. racialLocations);
						--Turbine.Shell.WriteLine("parametre 3 : racial" );
					...
					if( CheckMariner(centerQS[i]:GetShortcut():GetData()))then
						CreateMapWindow(ReturnIndex(settings["shortcuts"]["Data" .. i], marinerLocations), "mariner");
						settings["isMapWindowVisible"]["value"] = true;
						MapWindow:SetVisible(true);
					end
...
------------------------------------------------------------------------------------------
function CheckRacial(value) -- 8 racial location
	for i=1, 8 do
		--Turbine.Shell.WriteLine("value : " .. value);
		--Turbine.Shell.WriteLine("i : " .. i);
		--Turbine.Shell.WriteLine("racialLocations[i] : " .. racialLocations[i]);

		if(value == racialLocations[i])then
			--Turbine.Shell.WriteLine("CHECKED !!! ");
			return true;
		end
	end
end
...
function CheckMariner(value)
	local valTmp = 0;
	valTmp = settings["marinerLoc"]["nbr"];
	for i=1, valTmp do
		if(value == marinerLocations[i])then
			return true;
		end
	end
end
...
function ReturnIndex(value, where)
	...
	if(where == marinerLocations)then
		valTmp = settings["marinerLoc"]["nbr"];
	end
